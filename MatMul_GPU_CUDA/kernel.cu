...
// from NVIDIA CUDA SDK [ REMEMBER : Some header file changes are done to the original file ]
...
__global__ void matrixMultiply(float *A, float *B, float *C, int numARows, int numAColumns, int numBRows, int numBColumns, int numCRows, int numCColumns)//CPU or GPU
{
	// variable declarations
...
int main(int argc, char *argv[])
{
	...
	void matMulHost(float *, float *, float *, int, int, int);// Host is of CPU
	void cleanup(void);
	...
	err = cudaMalloc((void **)&deviceA, sizeA); //Size A 's memory and give address A
	if (err != cudaSuccess)
	{
